...
fn build(mut cx: FunctionContext) -> JsResult<JsObject> {
	...
	let target = debrix_shared::int_to_target(cx.argument::<JsNumber>(1)?.value(&mut cx) as usize);

	let result = match debrixc::build(input, target) {
		Ok(result) => result,
		Err(err) => {
			return match err {
				debrixc::Error::ParserError(err) => {
				...
				debrixc::Error::CompilerError(err) => {
